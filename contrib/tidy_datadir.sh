...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete V Core database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no V Core datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old V Core datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected V Core 0.7 datadir."
    ;;
  3)
    echo "Detected V Core pre-0.8 datadir."
    ;;
  4)
    echo "Detected V Core 0.8 datadir."
    ;;
esac
