...
MultisigInputEntry::MultisigInputEntry(QWidget *parent) :
    QFrame(parent),
    ui(new Ui::MultisigInputEntry),
    model(0)
{
    ui->setupUi(this);
}
...
void MultisigInputEntry::on_transactionId_textChanged(const QString &transactionId)
{
    ...
    for (size_t i=0; i < tx->vout.size(); i++)
    {
        ...
        if(ExtractDestination(script, addr))
        {
            ...
            ui->transactionOutput->addItem(idStr + QString(" - ") + addressStr + QString(" - ") + amountStr + QString(" DTC"));
        }
        else
            ui->transactionOutput->addItem(idStr + QString(" - ") + amountStr + QString(" DTC"));
    }
}
