...
{
    this->model = model;
    updateMultisigAddressEntries(model);
    updateAmounts();
}

void MultisigDialog::updateMultisigAddressEntries(WalletModel *model) {
...
void MultisigDialog::showDialog() {
    this->show();
}
...
void MultisigDialog::on_createAddressButton_clicked()
{
    ...
    if((required == 0) || (required > pubkeys.size())) {
        ui->requiredSignatures->setValid(false);
        return;
    }
...
void MultisigDialog::on_transaction_textChanged()
{
    ...
    int64_t minFee = (int64_t)(transactionSize * (CENT / 1000));
    if(fee < minFee)
    {
...
//        return;
    ...
    ui->statusLabel->setText(tr("The transaction is sent to the V Core network."));
}
...
void MultisigDialog::updateAmounts()
{
    ...
    inputsAmountStr.sprintf("%.8f", (double) inputsAmount / COIN);
    ...
    outputsAmountStr.sprintf("%.8f", (double) outputsAmount / COIN);
    ui->outputsAmount->setText(outputsAmountStr);

    // Update fee amount
    int64_t fee = inputsAmount - outputsAmount;
    QString feeStr;
    feeStr.sprintf("%.8f", (double) fee / COIN);
    ui->fee->setText(feeStr);
}

void MultisigDialog::on_requiredSignaturesChanged()
{
    unsigned int required = ui->requiredSignatures->text().toUInt();
