...
/***/ function(module, exports) {
	...
	function convertPassphraseToKey(passphraseString, saltBase64) {
	    ...
	    return window.crypto.subtle.importKey("raw", passphraseBytes, { name: "PBKDF2" }, false, ["deriveKey"]).then(function (baseKey) {
	        return window.crypto.subtle.deriveKey(
	        // Firefox currently only supports SHA-1 with PBKDF2
	        { name: "PBKDF2", salt: saltBytes, iterations: iterations, hash: "SHA-1" }, baseKey, { name: "AES-CBC", length: 256 }, // Resulting key type we want
	        true, // exportable
	        ["encrypt", "decrypt"]);
	    }).catch(function (err) {
	...
	function decryptPrivateKey(encryptedPrivaeKey, passphrase) {
	    ...
	    convertPassphraseToKey(passphrase, base64Salt).then(function (key) {
	        ...
	        window.crypto.subtle.encrypt({ name: "AES-CBC", iv: iv }, key, ciphertextBytes).then(function (plaintextBuf) {
	...
	function encryptPrivateKey(privateKey, passphrase) {
	    ...
	    convertPassphraseToKey(passphrase, base64Salt).then(function (key) {
	        ...
	        window.crypto.subtle.encrypt({ name: "AES-CBC", iv: iv }, key, plaintextBytes).then(function (ciphertextBuf) {
	            // Encode ciphertext to base 64 and put in Ciphertext field
	...
	function createKeyPair(passphrase) {
	    window.crypto.subtle.generateKey({
	        ...
	        myKeys = keyPair;
	        window.crypto.subtle.exportKey("spki", keyPair.publicKey).then(function (spkiBuffer) {
	        ...
	        window.crypto.subtle.exportKey("pkcs8", keyPair.privateKey).then(function (pkcs8Buffer) {
	...
	function encrypt(inputPlaintext) {
	    ...
	    window.crypto.subtle.importKey("spki", spkiBytes, { name: "RSA-OAEP", hash: "SHA-256" }, false, ["encrypt"]).then(function (publicKey) {
	        // Now we need to create a random session key for encrypting
	        // the actual plaintext.
	        return window.crypto.subtle.generateKey({ name: "AES-CBC", length: 256 }, true, ["encrypt", "decrypt"]).then(function (sessionKey) {
	            ...
	            function encryptReadFile(inputPlaintext) {
	                ...
	                window.crypto.subtle.encrypt({ name: "AES-CBC", iv: ivBytes }, sessionKey, plaintextBytes).then(function (ciphertextBuffer) {
	            ...
	            window.crypto.subtle.exportKey("raw", sessionKey).then(function (sessionKeyBuffer) {
	                // Encrypt the session key in the buffer, save the encrypted
	                // key in the keyBox element.
	                window.crypto.subtle.encrypt({ name: "RSA-OAEP" }, publicKey, // from closure
	                sessionKeyBuffer).then(function (encryptedSessionKeyBuffer) {
	...
	function decrypt(message) {
	    ...
	    window.crypto.subtle.importKey("pkcs8", pkcs8Bytes, { name: "RSA-OAEP", hash: "SHA-256" }, false, ["decrypt"]).then(function (privateKey) {
	        ...
	        window.crypto.subtle.decrypt({ name: "RSA-OAEP" }, privateKey, encryptedSessionKeyBytes).then(function (sessionKeyBuffer) {
	
	            window.crypto.subtle.importKey(
	            // We can't use the session key until it is in a CryptoKey object
	            "raw", sessionKeyBuffer, { name: "AES-CBC", length: 256 }, false, ["decrypt"]).then(function (sessionKey) {
	                ...
	                window.crypto.subtle.decrypt({ name: "AES-CBC", iv: ivBytes }, sessionKey, ciphertextBytes).then(function (plaintextBuffer) {
