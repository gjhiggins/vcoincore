...
void ChatClient::errorSocket(QAbstractSocket::SocketError error)
{
    switch(error)
    {
        case QAbstractSocket::HostNotFoundError:
            utterance->append(tr("<em>ERROR : can't find freenode server.</em>"));
            break;
        case QAbstractSocket::ConnectionRefusedError:
            utterance->append(tr("<em>ERROR : server refused connection</em>"));
            break;
        case QAbstractSocket::RemoteHostClosedError:
            utterance->append(tr("<em>ERROR : server cut connection</em>"));
            break;
        default:
            utterance->append(tr("<em>ERROR : ") + this->errorString() + tr("</em>"));
    }
}

void ChatClient::connected()
{
    utterance->append("Connecting...");

    sendData("USER " + pseudo + " localhost " + server + " :" + pseudo);
    sendData("NICK " + pseudo);
    utterance->append("Connected to freenode.");
}
...
void ChatClient::readClient()
{
    ...
    Q_FOREACH(QString msg, msglist)
    {
        if(msg.contains(QRegExp(":([a-zA-Z0-9]+)\\![~a-zA-Z0-9]+@[a-zA-Z0-9\\/"
                                "\\.-]+ PRIVMSG ([a-zA-Z0-9\\#]+) :(.+)")))
        {
            QRegExp reg(":([a-zA-Z0-9]+)\\![~a-zA-Z0-9]+@[a-zA-Z0-9\\/\\.-]+ PRIVMSG "
                        "([a-zA-Z0-9\\#]+) :(.+)");
            QString msg2 = msg;
            write(msg.replace(reg, "\\2 <b>&lt;\\1&gt;</b> \\3"), "",
                   msg2.replace(reg, "\\2 <\\1> \\3"));
        }
        else if(msg.contains(QRegExp(":([a-zA-Z0-9]+)\\![~a-zA-Z0-9]+@[a-zA-Z0-"
                                     "9\\/\\.-]+ JOIN ([a-zA-Z0-9\\#]+)")))
        {
            QRegExp reg(":([a-zA-Z0-9]+)\\![~a-zA-Z0-9]+@[a-zA-Z0-9\\/\\.-]+ JOIN "
                        "([a-zA-Z0-9\\#]+)");
            QString msg2 = msg;
            write(msg.replace(reg, "\\2 <i>-> \\1 join \\2</i><br />"), "",
                   msg2.replace(reg, "-> \\1 join \\2"));
            updateUsersList(msg.replace(reg, "\\2"));
        }
        else if(msg.contains(QRegExp(":([a-zA-Z0-9]+)\\![~a-zA-Z0-9]+@[a-zA-Z0-"
                                     "9\\/\\.-]+ PART ([a-zA-Z0-9\\#]+)")))
        {
            QRegExp reg(":([a-zA-Z0-9]+)\\![~a-zA-Z0-9]+@[a-zA-Z0-9\\/\\.-]+ PART "
                        "([a-zA-Z0-9\\#]+) :(.+)");
            QString msg2 = msg;
            write(msg.replace(reg, "\\2 <i>-> \\1 quit \\2 (\\3)</i><br />"), "",
                   msg2.replace(reg, "-> \\1 quit \\2"));
            updateUsersList(msg.replace(reg, "\\2"));
        }
        else if(msg.contains(QRegExp(":([a-zA-Z0-9]+)\\![~a-zA-Z0-9]+@[a-zA-Z0-"
                                     "9\\/\\.-]+ QUIT (.+)")))
        {
            QRegExp reg(
                ":([a-zA-Z0-9]+)\\![~a-zA-Z0-9]+@[a-zA-Z0-9\\/\\.-]+ QUIT (.+)");
            QString msg2 = msg;
            write(msg.replace(reg, "\\2 <i>-> \\1 quit this server (\\2)</i><br />"),
                   "", msg2.replace(reg, "-> \\1 left"));
            updateUsersList(msg.replace(reg, "\\2"));
        }
        else if(msg.contains(QRegExp(":([a-zA-Z0-9]+)\\![~a-zA-Z0-9]+@[a-zA-Z0-"
                                     "9\\/\\.-]+ NICK :(.+)")))
        {
            QRegExp reg(
                ":([a-zA-Z0-9]+)\\![~a-zA-Z0-9]+@[a-zA-Z0-9\\/\\.-]+ NICK :(.+)");
            QString msg2 = msg;
            write(msg.replace(reg, "<i>\\1 is now called \\2</i><br />"), "",
                   msg2.replace(reg, "-> \\1 is now called \\2"));
            updateUsersList(currentChan);
        }
        else if(msg.contains(
                    QRegExp(":([a-zA-Z0-9]+)\\![a-zA-Z0-9]+@[a-zA-Z0-9\\/\\.-]+ "
                            "KICK ([a-zA-Z0-9\\#]+) ([a-zA-Z0-9]+) :(.+)")))
        {
            QRegExp reg(":([a-zA-Z0-9]+)\\!~[a-zA-Z0-9]+@[a-zA-Z0-9\\/\\.-]+ KICK "
                        "([a-zA-Z0-9\\#]+) ([a-zA-Z0-9]+) :(.+)");
            QString msg2 = msg;
            write(msg.replace(reg, "\\2 <i>-> \\1 kicked \\3 (\\4)</i><br />"), "",
                   msg2.replace(reg, "-> \\1 quit \\3"));
            updateUsersList(msg.replace(reg, "\\2"));
        }
        else if(msg.contains(
                    QRegExp(":([a-zA-Z0-9]+)\\![a-zA-Z0-9]+@[a-zA-Z0-9\\/\\.-]+ "
                            "NOTICE ([a-zA-Z0-9]+) :(.+)")))
        {
            ...
            {
                QRegExp reg(":([a-zA-Z0-9]+)\\![~a-zA-Z0-9]+@[a-zA-Z0-9\\/\\.-]+ "
                            "NOTICE [a-zA-Z0-9]+ :(.+)");
                write(msg.replace(reg, "<b>[NOTICE] <i>\\1</i> : \\2 <br />"),
                       currentChan);
            }
...
QString ChatClient::parseCommand(QString comm, bool chatclient)
{
    ...
    else if(!chatclient)
    {
        ...
        write("<b>&lt;" + pseudo + "&gt;</b> " + comm, destChan);
...
{
    utterance->append("Joining " + chan + " channel");
    Q_EMIT joinTab();
...
void ChatClient::write(QString txt, QString destChan, QString msgTray)
{
    if(destChan != "")
    {
    ...
    {
        txt.replace("\r\n", "<br />");
        utterance->setHtml(utterance->toHtml() + txt + "<br />");
        QScrollBar* sb = utterance->verticalScrollBar();
        sb->setValue(sb->maximum());
    }
}
...
void ChatClient::updateUsersList(QString chan, QString message)
{
    ...
    if(chan != server)
    {
        ...
        {
            QString _msg = message.replace(":", "");
            QStringList arglist = _msg.split(" ");

            if(delist == true)
                users.clear();

            for(int i = 5; i < arglist.count(); i++)
            {
                users.append(arglist.at(i));
            }
            updateUsers = false;
            if(arglist.count() < 53)
                delist = true;
