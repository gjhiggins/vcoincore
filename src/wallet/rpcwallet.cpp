...
static CTransactionRef SendMoney(interfaces::Chain::Lock& locked_chain, CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, const CCoinControl& coin_control, mapValue_t mapValue)
{
    ...
    if (mapValue.count("inscription")) {
        std::string dataMsg = mapValue["inscription"];
        mapValue.erase("inscription");
        std::vector<unsigned char> data(dataMsg.begin(), dataMsg.end());
        CRecipient recipient = {CScript() << OP_RETURN << data, nValue, fSubtractFeeFromAmount};
...
static UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"sendtoaddress",
                "\nSend an amount to a given address, optionally inscribing a TrustyURI on the blockchain." +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send to."},
                    {"amount", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The amount in " + CURRENCY_UNIT + " to send. eg 0.1"},
                    {"data", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "A TrustyURI to be inscribed.\n"},
            ...
            "\"txid\"                  (string) The transaction id.\n"
                ...
                RPCExamples{
                    HelpExampleCli("sendtoaddress", "\"VM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 \"ni://example.org/sha-256;5AbXdpz5DcaYXCh9l3eI9ruBosiL5XDU3rxBbBaUO70\"")
            + HelpExampleCli("sendtoaddress", "\"VM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 \"ni://example.org/sha-256;5AbXdpz5DcaYXCh9l3eI9ruBosiL5XDU3rxBbBaUO70\" \"notarize document\" \"resume\"")
            + HelpExampleCli("sendtoaddress", "\"VM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 \"ni://example.org/sha-256;5AbXdpz5DcaYXCh9l3eI9ruBosiL5XDU3rxBbBaUO70\" \"\" \"\" true")
            + HelpExampleRpc("sendtoaddress", "\"VM72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", 0.1, \"ni://example.org/sha-256;5AbXdpz5DcaYXCh9l3eI9ruBosiL5XDU3rxBbBaUO70\", \"notarize document\", \"resume\"")
                },
            }.Check(request);
    ...
    // TrustyURI and Wallet comments
    mapValue_t mapValue;
    if (!request.params[2].isNull() && !request.params[2].get_str().empty())
        mapValue["inscription"] = request.params[2].get_str();
...
static UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
            ...
            "                         Must be at least 0.0001 VCN per kB higher than the current transaction fee rate.\n"},
...
UniValue makekeypair(const JSONRPCRequest& request); // in rpcdump.cpp
// UniValue dumpbootstrap(const JSONRPCRequest& request); // in rpcdump.cpp
...
{ //  category              name                                actor (function)                argNames
    ...
    { "wallet",             "sendtoaddress",                    &sendtoaddress,                 {"address","amount","inscription","comment","comment_to","subtractfeefromamount","replaceable","conf_target","estimate_mode","avoid_reuse"} },
